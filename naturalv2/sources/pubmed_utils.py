...
def fetch_articles(webenv, query_key, api_key, data_path):
    ...
    for article in root.findall(".//PubmedArticle"):
        ...
        article_data['abstract'] = article_data['abstract'].text if article_data['abstract'] is not None else "No abstract available"
        article_data['abstract'] = article_data['abstract'] if article_data['abstract'] is not None else "No abstract available"
        # Extract authors
        ...
        if pub_date is not None:
            ...
            article_data['publication_date'] = f"{year.text if year is not None else ''}-{month.text if month is not None else ''}-{day.text if day is not None else ''}"
        ...
        if article_id_list is not None:
            for article_id in article_id_list.findall("ArticleId"):
                if article_id.get("IdType") == "pmc":
                    ...
                    try:
                        article_data['full_text'] = fetch_pmc_fulltext(data_path, article_data['pmc_id'], api_key)
                    except Exception as e:
                        print(f"Error retrieving full text for PMC{article_data['pmc_id']}: {str(e)}")
                    break        
...
def parse_section(section):
    ...
    for child in section:
        if child.tag == 'p':
            content.append(ElementTree.tostring(child, encoding="unicode", method="text").strip())
        elif child.tag == 'sec':
