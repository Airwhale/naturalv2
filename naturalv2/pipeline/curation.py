...
class CurationStage(ABC):
    ...
    def prompt_template(self):
        ...
        if self.extract_type:
            ...
            filepath = os.path.join(prompts_dir, f"{self.extract_type}_{self.source_name}.yaml")
...
class ConditionStage(CurationStage):
    ...
    def __init__(
        self, model_cfg: DictConfig, source_name: str, max_concurrent_workers: int | None = None,
    ) -> None:
    ...
    async def process(self, exp_list: list[Experiment], context: CurationContext) -> list[str]:
        ...
        condition_queries = await context.source_dataset.condition_filter(all_condition_keywords)
...
class SynonymStage(CurationStage):
    ...
    def __init__(
        self, model_cfg: DictConfig, source_name: str, attribute: str, max_concurrent_workers: int | None = None,
    ) -> None:
        """Initialize the class."""
        super().__init__(model_cfg, source_name)
        self.source_name = source_name
    ...
    async def process(self, exp_list: list[Experiment], context: CurationContext) -> list[Experiment]:
       ...
       list[Experiment]
        ...
        for exp in exp_list:
            for keyword in getattr(exp, f"{self.attribute}_names"):
                if self.source_name not in getattr(exp, f"{self.attribute}_common_names"):
                    getattr(exp, f"{self.attribute}_common_names").update({
                        self.source_name: {}
                    })
                if keyword in getattr(exp, f"{self.attribute}_common_names")[self.source_name]:
                    logger.debug(
        ...
        save_dir = os.path.join(context.save_path, "curation_results")
        os.makedirs(save_dir, exist_ok=True)
        file_path = os.path.join(save_dir, f"{self.attribute}_synonyms_{context.exp_name}.csv")
        ...
        for exp in exp_list:
            ...
            getattr(exp, f"{self.attribute}_common_names")[self.source_name].update(synonyms_dict)
