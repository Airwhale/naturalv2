...
class NaturalIPW:
    ...
    def compute_prop_score(self, conditionals):
        ...
        for i in range(len(feat_dicts)):
            ...
            if len(subset) == 0:
                prop_scores = [0 for _ in self.num_treat]
            else:
                # marginalize out Y
                ...
                for t in range(self.num_treat):
                    ...
                    prop_sores.append(prop_t)        
            prop_score_lst.append(prop_scores)
        return prop_score_lst

    def get_ite(self, conditionals, outcome):
        ...
        conditionals.loc[:, "probs"] = conditionals.apply(
            lambda arr: arr[:, 2*outcome_idx: 2*(outcome_idx+1)]
        )
        
        self.prop_score_lst = self.compute_prop_score(conditionals)
        all_ites = np.zeros(num_treat, len(conditionals))
        ...
        for i, row in enumerate(conditionals.iterrows()):
            ...
            for t in range(self.num_treat):
                ...
                for y in range(2):
                    ...
                    z_given_x = self.prop_score_lst[i][t]
                    # ignore propensity sores of 0
                    if 0 not in z_given_x:     
                        all_ites[t, i] = z * y * posterior / z_given_x                     
