...
class SvT:
        ...
        treatment = (
            ...
            .apply(
                ...
                in [
                    ...
                    "Wegovy/Semaglutide",
                    "Wegovy - Semaglutide",
                    "GLP-1 agonist medications (Semaglutide)",
                    ...
                    "Wegovy/Ozempic",
                    "Wegovy / Ozempic",
                    "Mix of Semaglutide (Wegovy) and Ozempic",
                    "Mixed (Wegovy and Ozempic)",
                    "Ozempic/Wegovy",
                    "Wegovy or Ozempic (Semaglutide)",
                    "Wegovy or Ozempic",
                    "Wegovy (under the Ozempic brand)",
                    "Wegovy and Ozempic",
    ...
    def hard_filter_inclusion(self, extract):
        ...
        dosage = (
            ((extract["treatment"] == 0) & (extract["dosage"] == 1))
            | ((extract["treatment"] == 1) & (extract["dosage"] == 5))
            | (extract["dosage"].isna())
        )
    ...
    def get_options(self, feature_names):
        options = {
            ...
            "treatment": [
                "Semaglutide like Ozempic or Wegovy or Rybelsus",
                "Tirzepatide like Mounjaro or Zepbound",
            ],  # treatment
            "target_achieved": ["No", "Yes"],  # outcome
            "inclusion": ["No", "Yes"],  # satisfies inclusion criteria
        }
    ...
    def get_question_prompt(self, feature_names):
        questions = {
            ...
            "treatment": "Which treatment did the user take?",  # treatment
            "target_achieved": "Was the user's final percentage weight change less than or equal to -5%?",  # outcome
            "inclusion": "Does the user satisfy the given inclusion criteria?",  # satisfies inclusion criteria
        }
    ...
    def transform_samples(self, dct):
        ...
        for field in all_keys:
            if field in ["sample_treatment", "treatment"]:
                binary_map = {
                    "Semaglutide like Ozempic or Wegovy or Rybelsus": 0,
                    "Tirzepatide like Mounjaro or Zepbound": 1,
                }
                dct[field] = binary_map[dct[field]]
            elif field in [
                "sample_target_achieved",
                "sample_age",
                "sample_bmi",
                "sample_start_HbA1c",
                "sample_start_weight",
    ...
    def interpret_samples(self, dct):
        ...
        for field in all_keys:
            if field in ["sample_sex", "sex"]:
                ...
                drug_map = {
                    0: "Semaglutide like Ozempic or Wegovy or Rybelsus",
                    1: "Tirzepatide like Mounjaro or Zepbound",
                }
    ...
    def discretize(self, sample_df, hard_filter=True, inf=True):
        ...
        for numeric_feat in [
            ...
            if numeric_feat in sample_df.columns:
                sample_df[numeric_feat] = pd.to_numeric(
                    sample_df[numeric_feat], errors="coerce"
                )
        ...
        compute_change = sample_df[
            (sample_df.percentage_weight_change.isna())
            & (sample_df.weight_change.isna())
        ].index  # our filtering ensures that start+end weight are non-NA for these
        sample_df.loc[compute_change, "weight_change"] = (
            sample_df.loc[compute_change, "end_weight"]
            - sample_df.loc[compute_change, "start_weight"]
        )
        compute_target = sample_df[sample_df.percentage_weight_change.isna()].index
        sample_df.loc[compute_target, "percentage_weight_change"] = 100 * (
            sample_df.loc[compute_target, "weight_change"]
            / sample_df.loc[compute_target, "start_weight"]
        )
        sample_df.loc[
            sample_df["percentage_weight_change"] <= -5, "target_achieved"
        ] = 1
        sample_df.loc[
            ~(sample_df["percentage_weight_change"] <= -5), "target_achieved"
        ] = 0
        ...
        sample_df = sample_df.drop(
            sample_df[~sample_df["treatment"].isin([0, 1])].index
        )
        ...
        if hard_filter:
            ...
            inf
        ):  # samples are already filtered; we need to discretize for causal inference
