...
def extract_data_type_from_filename(csv_file):
    ...
    data_types = sorted(["ty_filter", "knowns", "imputations", "inclusion", "ty_given_x", "sample_ty"], 
...
def sort_files_by_data_type(files_list):
    ...
    data_type_order = ["ty_filter", "knowns", "imputations", "sample_ty", "inclusion", "ty_given_x"]
...
def get_default_columns(data_type, all_columns, covariate_names=None):
    ...
    if data_type == "ty_filter":
        defaults = ["report", "treatments_mentioned", "treatment_taken_filter", "outcome_category_filter"]
...
def main():
    ...
    data_path = st.sidebar.text_input(
        "Data Path", value="/mfs1/u/nikita/naturalv2"
    )
    experiment_name = st.sidebar.text_input(
        "Experiment Name", value="_gpt_gemini"
    )
    ...
    try:
        ...
        if nct_id:
            ...
            if "notes_dict" not in st.session_state or st.session_state.get(notes_path_key) != data_path:
            ...
            with col1:
                ...
                if experiment:
                    ...
                    if available_fields:
                        ...
                        field_cols = st.columns(min(len(available_fields), 5), gap="small")
                        for idx, field in enumerate(available_fields):
                            col_idx = idx % min(len(available_fields), 5)
                            with field_cols[col_idx]:
                                if st.button(field, key=f"field_btn_{nct_id}_{field}", width='content'):
                    ...
                    if selected_field:
                        ...
                        if field_value is not None:
                            ...
                            st.error(f"Field '{selected_field}' not found or not accessible.")
                else:
                ...
                elif st.session_state[notes_key] != current_saved_notes and st.session_state.get(notes_saved_key, False):
            ...
            with col2:
                ...
                if len(figures) == 0:
                    ...
                    results_dir = os.path.join(data_path, "results")
                    nct_results_dir = os.path.join(results_dir, f"{nct_id}{experiment_name}")
                    ...
                    if os.path.exists(nct_results_dir):
                        ...
                        results_csvs = []
                        if os.path.exists(os.path.join(nct_results_dir, "apo_results.csv")):
                            results_csvs.append("apo_results.csv")
                        if os.path.exists(os.path.join(nct_results_dir, "ate_results.csv")):
                            results_csvs.append("ate_results.csv")
                        
                        if results_csvs:
                            # Display buttons for results CSVs
                            results_cols = st.columns(min(len(results_csvs), 4), gap="small")
                            for idx, csv_file in enumerate(results_csvs):
                                col_idx = idx % min(len(results_csvs), 4)
                                with results_cols[col_idx]:
                                    if st.button(csv_file, key=f"results_csv_btn_{nct_id}_{csv_file}", width='content'):
                                        st.session_state[f"selected_results_csv_{nct_id}"] = csv_file
                            
                            # Display selected results CSV
                            selected_results_csv = st.session_state.get(f"selected_results_csv_{nct_id}")
                            if selected_results_csv:
                                csv_path = os.path.join(nct_results_dir, selected_results_csv)
                                try:
            ...
            with data_col1:
                ...
                if os.path.exists(nct_results_dir):
                    ...
                    all_csv_files = [f for f in os.listdir(nct_results_dir) if f.endswith('.csv')]
                    csv_files = [f for f in all_csv_files if f not in ["apo_results.csv", "ate_results.csv"]]
                    ...
                    if csv_files:
                        ...
                        for csv_file in csv_files:
                            ...
                            for data_type in data_types:
                                ...
                                if idx != -1:
                                    ...
                                    data_type_start = idx + 1  # Start of data_type (after first underscore)
                                    data_type_end = idx + len(pattern)  # End of pattern (after second underscore)
                                    break
                            ...
                            if found_data_type and data_type_start != -1:
                                ...
                                model_data_type = name_without_ext[:data_type_end - 1]  # -1 to remove trailing underscore
                                
                                if outcome not in outcome_groups:
                                    outcome_groups[outcome] = []
                                outcome_groups[outcome].append((model_data_type, csv_file))
                            else:
                        ...
                        for outcome in sorted(outcome_groups.keys()):
                            st.write(f"**{outcome}**")
                            files_for_outcome = sort_files_by_data_type(outcome_groups[outcome])
                            csv_cols = st.columns(min(len(files_for_outcome), 6), gap="small")
                            for idx, (model_data_type, csv_file) in enumerate(files_for_outcome):
                                col_idx = idx % min(len(files_for_outcome), 6)
                                with csv_cols[col_idx]:
                                    if st.button(model_data_type, key=f"csv_btn_{nct_id}_{csv_file}", width='content'):
                                        st.session_state[f"selected_csv_{nct_id}"] = csv_file
                                        # Load dataframe immediately when button is clicked
                                        csv_path = os.path.join(nct_results_dir, csv_file)
                                        try:
                                            df = pd.read_csv(csv_path)
                                            st.session_state[f"df_{nct_id}_{csv_file}"] = df
                                            # Initialize default columns
                                            selected_cols_key = f"selected_cols_{nct_id}_{csv_file}"
                                            if selected_cols_key not in st.session_state:
                                                data_type = extract_data_type_from_filename(csv_file)
                                                experiment = st.session_state.get(f"experiment_{nct_id}")
                                                covariate_names = experiment.covariate_names if experiment else None
                                                default_cols = get_default_columns(data_type, list(df.columns), covariate_names)
                                                st.session_state[selected_cols_key] = default_cols if default_cols else list(df.columns)
                                        except Exception as e:
                                            st.error(f"Error loading CSV file: {str(e)}")
                        ...
                        if other_files:
                            ...
                            other_cols = st.columns(min(len(other_files), 6), gap="small")
                            for idx, csv_file in enumerate(sorted(other_files)):
                                col_idx = idx % min(len(other_files), 6)
                                with other_cols[col_idx]:
                                    if st.button(csv_file, key=f"csv_btn_{nct_id}_{csv_file}", width='content'):
                                        st.session_state[f"selected_csv_{nct_id}"] = csv_file
                                        # Load dataframe immediately when button is clicked
                                        csv_path = os.path.join(nct_results_dir, csv_file)
                                        try:
                ...
                if selected_csv:
                    ...
                    if df_key in st.session_state:
                        ...
                        if current_selection is None:
                            ...
                            experiment = st.session_state.get(f"experiment_{nct_id}")
                            covariate_names = experiment.covariate_names if experiment else None
                            current_selection = get_default_columns(data_type, list(df.columns), covariate_names)
                        ...
                        valid_selection = [col for col in current_selection if col in df.columns]
                        ...
                        for idx, col in enumerate(df.columns):
                            ...
                            with checkbox_cols[col_idx]:
                                checkbox_value = st.checkbox(col, value=is_selected, key=f"col_check_{nct_id}_{selected_csv}_{col}")
                                if checkbox_value:
            ...
            if selected_csv:
                ...
                if os.path.exists(nct_results_dir):
                    ...
                    try:
                        ...
                        selected_cols_key = f"selected_cols_{nct_id}_{selected_csv}"
                        selected_cols = st.session_state.get(selected_cols_key, list(df.columns))
                        display_df = df[selected_cols] if selected_cols else df
                        
                        st.write(f"**{selected_csv}**:")
                        # Display random sample
                        sample_size = min(20, len(display_df))
                        sample_df = display_df.sample(n=sample_size, random_state=42) if len(display_df) > sample_size else display_df
                        st.dataframe(sample_df, width='stretch')
                        st.write(f"Total rows: {len(df)}, Columns: {len(display_df.columns)}")
                    except Exception as e:
                        st.error(f"Error loading CSV file: {str(e)}")
