    ...
    for subdir in os.listdir(results_dir):
        ...
        if os.path.isdir(subdir_path) and subdir.endswith(experiment_name):
            ...
            if os.path.exists(apo_csv_path):
                ...
                subset_cols = [c for c in ["treatment", "outcome", "estimator"] if c in df.columns]
...
def create_forest_plots(final_df, data_path, output_path, experiment_name="_gpt_gemini"):
    ...
    with PdfPages(output_path) as pdf:
        for (nct_id, outcome), group_df in grouped:
            ...
            for i, (idx, row) in enumerate(group_df.iterrows()):
                ...
                ax.errorbar(pred_response, y_pos, 
                           xerr=[[lower_err], [upper_err]], 
                           color='purple', linewidth=2, alpha=0.6, 
                           capsize=5, capthick=2, fmt='none', zorder=2)
                
                ax.scatter(pred_response, y_pos, color='purple', marker='o', s=80, 
                            zorder=3, label='NATURAL-IPW Response' if i == 0 else '')
                ax.scatter(row['true_response'], y_pos, color='green', marker='s', s=80, 
                            zorder=3, label='Clinical Trial Response' if i == 0 else '')
                
                # Add imputations_data_count below purple scatter point
                imp_count = row.get('imputations_data_count')
                if pd.notna(imp_count):
                    ax.text(pred_response, y_pos - 0.08, f'{int(imp_count)}',
                ...
                if pd.notna(cohort_size):
                    ax.text(row['true_response'], y_pos + 0.17, f'{int(cohort_size)}',
                           ha='center', va='top', fontsize=12, zorder=4)
