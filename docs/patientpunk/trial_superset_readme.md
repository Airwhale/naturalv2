...
> **Historical.** The original scope note for the `trial_superset/` work, kept for its
> depend-don't-reproduce rationale, which still stands. Its headline count — "9 fit NATURAL's
> premise" — predates the coverage-first correction; the current figure is **19** completed
> trials plus 3 prospective targets, see [benchmark_19.md](benchmark_19.md). The code it
> describes now lives in `patientpunk/analysis/`.
