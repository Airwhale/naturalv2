...
> **Historical draft.** Written before the coverage-first trial selection (which took the set
> from 5 to 19 trials) and before the 2026-08-09 re-pin that fixed A2 and A3. Numbers here are
> superseded. Kept for its description of the pull process and the CSV contents; for current
> status read [findings.md](findings.md) and [pipeline_overview.md](pipeline_overview.md).
