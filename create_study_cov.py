...
from naturalv2.utils import check_trial, get_nested_value, ListResponse, load_prompt
...
def get_covariates_from_llm(condition: str, lm_cfg: DictConfig) -> list[str]:
    lm = build_lm_instance_from_cfg(lm_cfg)

    messages: list[dict[str, str]] = load_prompt(
        base_dir="naturalv2/prompts",
        prompt_type="condition_covariates",
        return_format="messages",
        condition=condition,
    )

    response = lm.call_sync(messages=messages, response_format=ListResponse)
    covariates = extract_list_response(response)
    if covariates:
        final_covaraites = covariates[0]
        logger.info(
            f"LLM extracted the following covariates for {condition}: {final_covaraites}"
        )
        return final_covaraites

    raise ValueError(
        "The LLM did not return any covariates. Please check the model and the prompt."
    )
...
class Study:
    def __init__(
        ...
        self.train_trials = [
            {exp.nct_id: [len(exp.effect_sizes), exp.title, exp.date] + exp.references}
        ...
        self.test_trials = [
            {exp.nct_id: [len(exp.outcome_treatment), exp.title, exp.date] + exp.references}
            for exp in test_exp
        ...
        self.llm_covariates = get_covariates_from_llm(self.conditions[0], cfg.sample_model)
